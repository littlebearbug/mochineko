...
- **🖥️ 内置后台**: 纯客户端 CMS 后台，直接在浏览器中管理 GitHub 上的文章。
...
- [React](https://reactjs.org/) (v19) – UI 库
- [TailwindCSS](https://tailwindcss.com/) (v4) – 实用优先的 CSS 框架
...
你可以选择通过内置的 CMS 后台或者命令行来创建文章。

### 方式一：使用 Admin 后台 (推荐)

访问 `/admin` 页面，登录后点击 "New Post" 即可在可视化编辑器中撰写文章。编辑器支持实时预览、元数据配置和草稿保存。
...
本项目包含一个 **纯客户端 (Client-side Only)** 的 CMS 后台，允许你直接管理 GitHub 仓库中的内容。

### ✨ 功能特点

- **无后端架构**: 直接使用 GitHub API，不需要额外的服务器或数据库。
- **安全**: Token 仅存储在你的本地浏览器 (Local Storage)，不会发送给任何第三方。
- **草稿系统**: 基于 Git 分支的草稿流程。保存草稿会自动创建一个 `content/slug` 分支，发布时自动创建 Pull Request 并合并到 `main`。
- **图片上传**: 支持直接复制粘贴或上传图片，自动提交到仓库。
...
2. **登录配置**:
   - **Owner**: 你的 GitHub 用户名。
   - **Repo**: 你的仓库名称 (例如 `mochineko`)。
   - **Token**: 你的 GitHub Personal Access Token (PAT)。
3. **获取 PAT**:
   - 前往 [GitHub Settings > Tokens](https://github.com/settings/tokens)。
   - 生成新 Token (Classic)，确保勾选 `repo` 权限 (用于读写仓库内容)。
